...
trait Thumbnail
{
    public function getThumbnail($id)
    {
        $sql = "SELECT *
                FROM tblimage
                WHERE id = '{$id}'";
        $image = self::$pdo->query($sql)->fetch();
        $thumb_url = UPLOAD_URL.'/'.$this->thumbnail($image['image_name'], API_UPLOAD_PATH, UPLOAD_PATH, 500, 500, false, true);
