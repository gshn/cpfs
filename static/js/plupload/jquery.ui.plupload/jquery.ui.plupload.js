		...
		'<div class="plupload_wrapper">' +
			'<div class="ui-widget-content plupload_container">' +
				...
				'<tr>' +
					...
					'<td class="plupload_file_size"><span class="plupload_total_file_loaded">0 kb</span>/<span class="plupload_total_file_size">0 kb</span></td>' +
                    '<td class="plupload_file_speed"><span class="plupload_total_file_speed">0 kb</span>/초</td>' +
                    '<td class="plupload_file_time"><span class="plupload_total_file_time">00:00</span>초 남음</td>' +
...
$.widget("ui.plupload", {
	...
	*/
	...
	_updateTotalProgress: function() {
        ...
        var time = 0;
        var loaded = Math.ceil(up.total.loaded);
        if (up.total.bytesPerSec > 0) {
            time = Math.ceil((up.total.size - loaded) / up.total.bytesPerSec);
        }

        time = this._pad(Math.floor(time / 60), 2) + ':' + this._pad(time % 60, 2);

		this.element
			.find('.plupload_total_status')
				.html(up.total.percent + '%')
				.end()
            .find('.plupload_total_file_loaded')
                .html(plupload.formatSize(loaded))
                .end()
			.find('.plupload_total_file_size')
				.html(plupload.formatSize(up.total.size))
				.end()
            .find('.plupload_total_file_speed')
				.html(plupload.formatSize(up.total.bytesPerSec))
				.end()
            .find('.plupload_total_file_time')
				.html(time)
				.end()
